...
//            new FSDatasetInstanceSingularizator.LAION_30M_GHP_50_512Dataset(publicQueries),
    ...
    private static void run(Dataset dataset) {
        AbstractMetricSpacesStorage abstractStorage = dataset.getMetricSpacesStorage();
        if (abstractStorage instanceof FSMetricSpacesStorage) {
            FSMetricSpacesStorage storage = (FSMetricSpacesStorage) abstractStorage;
            try {
                storage.evaluateAndStoreNumberOfObjectsInDataset(dataset.getDatasetName());
            } catch (Exception e) {
